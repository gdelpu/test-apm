...
// Load YAML config for root_page_id
let ROOT_PAGE_ID_FROM_CONFIG = null;
const CONFIG_PATH = path.join(__dirname, 'confluence-config.yaml');
if (fs.existsSync(CONFIG_PATH)) {
  const configContent = fs.readFileSync(CONFIG_PATH, 'utf8');
  const rootMatch = configContent.match(/root_page_id:\s*"?(\d+)"?/);
  if (rootMatch) ROOT_PAGE_ID_FROM_CONFIG = rootMatch[1];
}
...
function savePagesRegistry(pages) {
  const lines = ['# Auto-generated by confluence-publish — do not edit manually', 'pages:'];
...
async function scaffoldConfluenceHierarchy() {
  ...
  // Update root_page_id in confluence-config.yaml
  if (fs.existsSync(CONFIG_PATH)) {
    ...
    console.log(`  ~ CONFIG  root_page_id=${rootId} written to confluence-config.yaml`);
  }
  ...
  savePagesRegistry(pages);
  console.log(`\nConfluence hierarchy ready. Registry: tools/confluence-pages.yaml`);
}
