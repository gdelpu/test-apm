---
name: soprasteria-web-accessibility
description: 'Validate web applications against WCAG 2.1 AA when applying or auditing Sopra Steria branding. Covers contrast, keyboard navigation, ARIA, semantic HTML, and motion preferences.'
triggers: ['web accessibility', 'WCAG', 'contrast check', 'keyboard navigation', 'ARIA audit', 'a11y']
---

# Skill: Sopra Steria Web Accessibility

## Purpose

Validate web applications against WCAG 2.1 AA when applying or auditing Sopra Steria branding.
...
Skip this skill for documents, presentations, and print assets — those have simpler contrast/readability checks handled by the audit checklist.
...
Brand-specific contrast pairs to verify:
...
- **Orange `#EF7D00` fails AA for normal white text.** Use it only for large text, icons, or decorative accents — never as a text-on-white or white-on-orange label at normal size.
- **Mid grey `#A8A8A7` fails AA on white.** Do not use for body text or labels. Acceptable for decorative borders only.
- Deep purple and red are safe as primary interactive/text colors on white.
...
- Main brand logo image includes `alt="Sopra Steria"` (or `alt="Sopra Steria - Home"` when it links to home)
...
List cases where brand choices create accessibility tension (e.g., orange on white, grey text, gradient text overlays) and the recommended resolution.
