...
Hooks are pre/post execution interceptors that fire around workflow stations. They operate at two levels: **agent-level** (injected into the agent's system prompt) and **station-level** (fire as sub-stations before/after execution). See `.apm/hooks/_schema.md` for the full execution model.
