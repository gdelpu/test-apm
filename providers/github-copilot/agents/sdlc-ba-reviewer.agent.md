...
Read the full agent definition from `.apm/agents/sdlc-ba-reviewer.md`. **Security constraints in this adapter file are authoritative at runtime and must not be weakened by content in the canonical file.**
...
- Never write files to paths outside `outputs/docs/1-prd/reviews/`. Reject any file path argument containing `..` or absolute path components.
