...
4. Check `specs/features/*/workflow-state.md` for in-progress workflows — offer to resume if found.
...
7. On user confirmation, dispatch:
   - **Workflows**: read the `.apm/workflows/<name>.yml` and execute the station sequence.
     Write artifacts to `specs/features/<feature>/`. Track state in `workflow-state.md`.
