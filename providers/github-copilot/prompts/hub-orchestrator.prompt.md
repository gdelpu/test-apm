...
4. Check `outputs/runs/*/latest/workflow-state.md` for in-progress workflows — offer to resume if found.
...
7. On user confirmation, dispatch:
   - **Workflows**: read the `.apm/workflows/<name>.yml` and execute the station sequence.
     Write artifacts to the run directory. Track state via `python -m engine --state` or `workflow-state.md`.
